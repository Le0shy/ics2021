...
static def_DHelper(B) {
  ...
  word_t imm = s->isa.instr.b.simm12 << 12
...
/* Add Decode Helper for J-type */
static def_DHelper(J) {
  word_t imm = s->isa.instr.j.imm20 << 20 
...
def_THelper(sr) {
  def_INSTR_TAB("?0????? ????? ????? ??? ????? ????? ??", srli);
  def_INSTR_TAB("?1????? ????? ????? ??? ????? ????? ??", srai);
...
def_THelper(main) {
  ...
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 00100 11", I     , opi);
  def_INSTR_IDTAB("??????0 ????? ????? ??? ????? 01100 11", R     , opr);
  def_INSTR_IDTAB("0000001 ????? ????? ??? ????? 01100 11", R     , opr_m);
